Public Class exceptions_prices_add
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ' fill filter ddls
            data.initDDlCountryAll(ddlCountry)
            data.initDDlCurrency(ddlCurrency)

            ' set selected values
            ddlCountry.Items.FindByValue("0").Selected = True
            ddlCurrency.Items.FindByValue("9").Selected = True
        ...
        'add first Return check box
        ReturnLocationList.Items.Add(New ListItem("Same as Pickup", 0))
        'add new check box for each item in the list
        For i As Integer = 0 To sqlDtLocs.Rows.Count - 1
            sqlDrLocs = sqlDtLocs.Rows(i)
    ...
    Protected Sub btnAddException_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        data.addExceptionsPrices(brandList, country, pickupList, returnList, OneWayRentals, startDate, endDate, priceChange, priceChangePercent, currency, priceChangeNote)
    ...
    '    data.updateDDLCountry(ddlCountry, 0)
